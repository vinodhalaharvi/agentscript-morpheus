...
type Command struct {
	Action string `@(
		...
		"fmap" | "pfmap" |
		"<>" |
		"gcp_check" | "deploy" | "schedule" | "undeploy" | "claude"
	)`
	Arg  string `@String?`
	Arg2 string `@String?`
	Arg3 string `@String?`
