...
type Config struct {
	...
	// ReadOnlyPaths lists path prefixes (relative to sandbox root) that the
	// engine MUST NOT write to. Attempts to write files or apply patches
	// under these prefixes are rejected and surface as apply-errors so the
	// reasoner sees the denial on the next loop and stops trying.
	//
	// Use case: generated code directories like "gen/" that must be produced
	// by real tools (buf, protoc, sqlc, ent, etc.) rather than by the LLM.
...
func (e *Engine) Run() error {
	...
	//
	// If the pipeline fails for ANY reason — parse error, exec failure,
	// MCP connect failure, anything — we abort immediately. Previously
	// the engine would print a warning and drop through to the intent
	// loop, which would then validate whatever stale state happened to
	// be in the sandbox from a previous run. That looks like success
	// ("all validations passed!") when in fact NONE of the pipeline
	// steps ran. Silent success theater. Fail loud instead.
	if e.config.PipelineDSL != "" {
		fmt.Println("▶ Running pipeline...")
		if _, err := e.runDSL(e.config.PipelineDSL); err != nil {
			fmt.Printf("\n❌ Pipeline FAILED — aborting before intent loop.\n")
			fmt.Printf("   Error: %v\n\n", err)
			fmt.Printf("   The intent loop is NOT being run because the pipeline\n")
			fmt.Printf("   did not complete. Any 'success' from a stale sandbox\n")
			fmt.Printf("   would be meaningless.\n\n")
...
func (e *Engine) collectContext() string {
	...
	if e.config.ContextDSL != "" {
		...
		if err != nil {
			// Surface context errors on stdout too — otherwise they're
			// buried in the prompt and the user never sees them.
			fmt.Printf("⚠️  Context DSL error (will be shown to reasoner): %v\n", err)
...
func (e *Engine) buildPrompt(attempt int, contextOutput string, results []ValidateResult) string {
	...
	if len(e.config.ReadOnlyPaths) > 0 {
		sb.WriteString("\n## READ-ONLY PATHS — the engine WILL REJECT writes here\n")
		for _, p := range e.config.ReadOnlyPaths {
			sb.WriteString(fmt.Sprintf("- %s (generated by an external tool — do not fabricate contents)\n", p))
		}
	}
...
// isReadOnly reports whether a sandbox-relative path falls under any
// configured ReadOnlyPaths prefix. Matches by path-segment prefix, so
// "gen" protects "gen/foo/bar.go" but not "generator/foo.go".
func (e *Engine) isReadOnly(path string) bool {
	clean := strings.TrimPrefix(filepath.Clean(path), "./")
	for _, prefix := range e.config.ReadOnlyPaths {
		p := strings.TrimPrefix(filepath.Clean(prefix), "./")
		if p == "" {
			continue
		}
...
func (e *Engine) applyProposal(proposal string) []string {
	...
	if len(prop.Patches) > 0 {
		for _, p := range prop.Patches {
			...
			if e.isReadOnly(p.File) {
				errMsg := fmt.Sprintf("refused to patch %s — path is read-only (generated by external tool; do NOT modify)", p.File)
				fmt.Printf("🔒 %s\n", errMsg)
	...
	if len(prop.Files) > 0 {
		for path, content := range prop.Files {
			...
			if e.isReadOnly(path) {
				errMsg := fmt.Sprintf("refused to write %s — path is read-only (generated by external tool; do NOT modify)", path)
				fmt.Printf("🔒 %s\n", errMsg)
				errors = append(errors, errMsg)
				continue
			}
